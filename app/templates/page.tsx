...
export default function FlowsPage() {
  ...
  async function createNewFlow() {
    if (!newFlowName.trim() || !flowContext.whoYouAre || !flowContext.whatOffering || !flowContext.whoTexting || !flowContext.infoNeeded || !flowContext.qualifyingQuestions) {
    ...
    try {
      ...
      if (data.steps) {
        ...
        setNewFlowName("");
        setFlowContext({ whoYouAre: "", whatOffering: "", whoTexting: "", infoNeeded: "", qualifyingQuestions: "", clientGoals: "" });
      ...
      {showNewFlowDialog && (
        <div className="card">
          <div className="space-y-4">
            ...
            <div>
              <label className="text-sm font-medium text-white mb-2 block">What information do you need from clients? *</label>
              <textarea
                placeholder="e.g., 'Age, current health status, income level, existing coverage, family size, smoker status'"
                value={flowContext.infoNeeded}
                onChange={e => setFlowContext({...flowContext, infoNeeded: e.target.value})}
                className="input-dark w-full px-4 py-3 rounded-lg resize-none"
                rows={2}
              />
              <p className="text-xs text-[var(--muted)] mt-1">What details help you qualify leads or provide accurate quotes?</p>
            </div>
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                disabled={isGenerating || !newFlowName.trim() || !flowContext.whoYouAre || !flowContext.whatOffering || !flowContext.whoTexting || !flowContext.infoNeeded || !flowContext.qualifyingQuestions}
              >
