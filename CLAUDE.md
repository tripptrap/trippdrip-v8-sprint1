...
### In Progress
- [ ] **Number-purchase checkout session never fulfills after payment** — found while fixing the capabilities bug above. When a user with no existing active Stripe subscription buys an extra phone number, `create-number-subscription/route.ts`'s checkout-session branch redirects to Stripe with `phone_number` in metadata, but the webhook's `checkout.session.completed` handler (`app/api/stripe/webhook/route.ts`) only reads `planType`/`points` metadata — it never orders the number from Telnyx or inserts into `user_telnyx_numbers`. Customer gets charged, number never shows up. Background task `task_ecdb8b8a` is fixing this in worktree `priceless-kowalevski-e9b46d` (branch `claude/priceless-kowalevski-e9b46d`).
...
- **Pre-Launch milestone**: everything blocking launch — 10DLC per-agent restructuring (#1) and its dependents (#2, #3, #11), the downgrade/retention flow (#12, #13, #14, #15), encryption of email API key (#16), end-to-end QA (#17), and production verification (#18). (#4 billing bug, #5 dark-mode pass, #6 /points balance, #7 floating button, #8 sidebar breakpoint, #9 analytics nav, and #10 number-order webhook are all closed — see issue history for what changed.)
